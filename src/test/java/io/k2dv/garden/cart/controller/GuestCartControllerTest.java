...
class GuestCartControllerTest {
    ...
    private CartResponse stubCart(UUID id) {
        ...
        return new CartResponse(id, CartStatus.ACTIVE, null, "USD", null,
            List.of(new CartItemResponse(UUID.randomUUID(), UUID.randomUUID(), 2,
                new BigDecimal("49.99"), productInfo, 1)),
            /* createdAt */ null);
    }
    ...
    void getCart_returns200() throws Exception {
        ...
        when(cartService.getOrCreateGuestCart(eq(SESSION_ID))).thenReturn(stubCart(cartId));
    ...
    @Test
    void abandonCart_returns204() throws Exception {
    ...
    void addItem_validRequest_returns200() throws Exception {
        when(cartService.addGuestItem(eq(SESSION_ID), any())).thenReturn(stubCart(UUID.randomUUID()));
    ...
    void updateItem_returns200() throws Exception {
        when(cartService.updateGuestItem(eq(SESSION_ID), any(), any()))
            .thenReturn(stubCart(UUID.randomUUID()));

        mvc.perform(put("/api/v1/guest-cart/items/{itemId}", UUID.randomUUID())
                .header("X-Guest-Session", SESSION_ID.toString())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void removeItem_returns200() throws Exception {
        ...
        mvc.perform(delete("/api/v1/guest-cart/items/{itemId}", UUID.randomUUID())
                .header("X-Guest-Session", SESSION_ID.toString()))
            .andExpect(status().isOk());
    }
}
