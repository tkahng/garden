...
Option `name` and option value `label` are mutable. Updating a value's label triggers a recomputation of all variant titles that use that value (via the `variant_option_values` join table). Updating an option's name does not require title recomputation because titles are built from value labels, not option names.
...
PATCH  /api/v1/admin/products/{id}/options/{optId}                  product:write  (rename option)
DELETE /api/v1/admin/products/{id}/options/{optId}                  product:delete
PATCH  /api/v1/admin/products/{id}/options/{optId}/values/{valId}   product:write  (rename value, recomputes variant titles)
...
- When an option value label is updated (`PATCH /options/{optId}/values/{valId}`), `OptionService` recomputes and saves the title of every non-soft-deleted variant linked to that option value via `variant_option_values`
...
- Rename option value label → verify all affected variant titles recomputed
